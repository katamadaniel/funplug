...
const EVENTS_API_URL = `${API_URL}/api/events/active`;
const VENUES_API_URL = `${API_URL}/api/venues/active`;
const PERFORMANCE_API_URL = `${API_URL}/api/performances/active`;
const SERVICES_API_URL = `${API_URL}/api/services/active`;
  ...
  const search = async (query = "") => {
    ...
    try {
      ...
      if (nearMe) {
        ...
        if (!loc.lat && !loc.city) {
          setSearchError("Please enable location access to use Near Me search.");
          return;
        }

        const params = {
          ...(loc.lat && loc.lng && { lat: loc.lat, lng: loc.lng, radius: 100 }),
      ...
      =============================== */
        ...
        results = [
          ...
          ...performanceResponse.data.map((p) => ({
            ...p,
            type: "performance",
          })),
