...
const PerformanceDetailsModal = ({ performance, onClose, onBookPerformance }) => {
  ...
  const [followerCount, setFollowerCount] = useState(
    performance?.userSnapshot?.followersCount || 0
  );

  const user = performance?.userSnapshot;

    const handleImageError = (e) => {
    e.target.src = process.env.REACT_APP_AVATAR_URL;
  };
  const shareUrl = window.location.href;
  const shareText = `Check out this performer: ${performance.artType}!`;
  ...
  const creatorId = performance?.userSnapshot?._id;
      ...
      <Dialog open={true} onClose={onClose} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          ...
          <Box position="relative">
            ...
            {/* FULLSCREEN BUTTON */}
            <IconButton
              onClick={() => setFullImage(performance.images[0].url)}
